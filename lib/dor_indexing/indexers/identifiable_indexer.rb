...
class DorIndexing
  module Indexers
    ...
    class IdentifiableIndexer
      attr_reader :cocina, :cocina_repository, :administrative_tags_repository

      CURRENT_CATALOG_TYPE = 'folio'

      def initialize(cocina:, cocina_repository:, administrative_tags_repository:, **)
        @cocina = cocina
      ...
      def populate_cache(rel_druid)
        @@apo_hash[rel_druid] ||= begin
          related_obj = cocina_repository.call(rel_druid)
      ...
      def hydrus_tag?(id)
        administrative_tags_repository.call(id).include?('Project : Hydrus')
      end
    end
  end
end
