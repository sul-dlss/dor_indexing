...
RSpec.describe DorIndexing::Indexers::ReleasableIndexer do
  ...
  describe 'to_solr' do
    let(:doc) { described_class.new(cocina:, parent_collections:, release_tags_repository:).to_solr }
    let(:release_tags_repository) { ->(_druid) { release_tags } }
    ...
    context 'with a parent collection' do
      ...
      let(:release_tags_repository) do
        lambda do |druid|
