...
RSpec.describe DorIndexing do
  describe '.build' do
    let(:doc) { described_class.build(cocina_with_metadata:, workflow_client:, cocina_repository:) }
    ...
    it 'builds the document' do
      expect(doc).to eq({})
      expect(DorIndexing::Builders::DocumentBuilder).to have_received(:for).with(model: cocina_with_metadata, workflow_client:,
                                                                                 cocina_repository:)
