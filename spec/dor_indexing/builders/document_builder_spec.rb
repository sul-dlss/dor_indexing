...
RSpec.describe DorIndexing::Builders::DocumentBuilder do
  subject(:indexer) do
    described_class.for(model: cocina_with_metadata, workflow_client:, cocina_repository:, administrative_tags_repository:, release_tags_repository:)
  end
  ...
  let(:cocina_repository) { ->(_druid) {} }
  let(:administrative_tags_repository) { ->(_druid) { [] } }
  let(:release_tags_repository) { ->(_druid) {} }
  ...
  context 'when the model is an item' do
    ...
    context 'with a cached collections' do
      ...
      before do
        allow(cocina_repository).to receive(:call).and_call_original
        described_class.for(
    ...
    context "with collections that can't be resolved" do
      let(:collections) { ['druid:bc999df2323'] }
      let(:cocina_repository) { ->(_druid) { raise DorIndexing::RepositoryError } }
